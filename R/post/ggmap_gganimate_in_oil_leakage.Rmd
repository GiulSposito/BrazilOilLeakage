...
Agora, vamos fazer uma animação usando o `gganimate package`.
...
anim <- p  +
  ...
  labs(title="Oil Sighting on Brazil Coast",
       subtitle = "Date:  {frame_time} - Fonte: IBAMA") +
...
## Visualizing the leakage evolution

A visualização animada no mapa dá uma idéia do tamanho do impacto ecológico do vazamento de óleo na costa do Brazil, mas a animação não permite ver todo o intervalo do tempo ao mesmo tempo para avaliar como o vazamento está progredindo.

Para tentar visualizar o intevalo todo vamos tentar um outro tipo de gráfico.
...
Neste gráfico, visualizamos o status de avistamento de óleo pelas comunidades ao longo da latitude (sul=mais baixo), e podemos visualizar que as manchas caminham para o Sul ao longo dos meses de outubro e novembro, atingindo o ponto mais extremo, poucos dias antes de dezembro.

A natureza intermitente do gráfico mostra que o registro de status não é feito diariamente em todas as localidades, e algumas comunidades são mais ficalizadas que outras.